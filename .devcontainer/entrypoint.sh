...
#   SSH_PUBLIC_KEY        Public key installed in ~node/.ssh/authorized_keys.
...
#                         Default: /home/node/ai-context.

set -u

NODE_USER="node"
NODE_HOME="/home/${NODE_USER}"
SSH_DIR="${NODE_HOME}/.ssh"
AUTH_KEYS="${SSH_DIR}/authorized_keys"

# ---- SSH authorized_keys ---------------------------------------------------
install -d -m 700 -o "${NODE_USER}" -g "${NODE_USER}" "${SSH_DIR}"
: > "${AUTH_KEYS}"
...
chmod 600 "${AUTH_KEYS}"
chown "${NODE_USER}:${NODE_USER}" "${AUTH_KEYS}"
...
if [ -n "${AI_CONTEXT_REPO:-}" ]; then
  AI_CONTEXT_DIR="${AI_CONTEXT_DIR:-${NODE_HOME}/ai-context}"
  target="${AI_CONTEXT_DIR}/$(basename "${AI_CONTEXT_REPO%.git}")"
  if [ ! -d "${target}" ]; then
    install -d -m 755 -o "${NODE_USER}" -g "${NODE_USER}" "${AI_CONTEXT_DIR}"
    echo "entrypoint: cloning ${AI_CONTEXT_REPO} -> ${target}"
    runuser -u "${NODE_USER}" -- git clone "${AI_CONTEXT_REPO}" "${target}" \
